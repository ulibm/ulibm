<?php 
	...
	$source_db=tmq("select * from fines_notif where setid='$setid' ",false);
	$num=number_format(tmq_num_rows($source_db));
	?><CENTER><?php 
	echo getlang("ส่งอีเมล์ทวงถามค่าปรับถึงผู้รับ $num ฉบับ::l::Send $num notification emails ");
	...
	<FORM METHOD=POST ACTION="emailsaction.php">
		...
		echo stripslashes(barcodeval_get("finesnotifnotif-email-mailsubj"));
		?>" size=60></TD>
	</TR>
	<TR>
		...
		echo stripslashes(barcodeval_get("finesnotifnotif-email-mailbody"));
		?></TEXTAREA></TD>
	</TR>
	<TR>
	...
	<TR>
		...
		[HOLDINGLIST]=<?php echo getlang("รายการค่าปรับค้างชำระ::l::Pending fine list");?><BR>
		[MYNAME]=<?php echo getlang("ชื่อของคุณ::l::Your name");?><BR>
		[LIBURL]=<?php echo getlang("URL เว็บไซต์::l::Website's URL");?><BR>
		[HOLDNUM]=<?php echo getlang("จำนวนรายการที่ยืมอยู่::l::Number of holding items");?><BR>
