<?php 
	; 
		...
		html_start();
$now=time();
	$showcase="select * from webpage_indexphotonews where cate='news' and dtstart<=$now and dtend>=$now order by dt desc";
...
width: <?php  echo $set_width-10?>px; /*width of featured content slider*/
height: <?php  echo $set_height-30?>px;
padding: 0 0 0 0;
}
...
padding: 0px;
background: white;
width: <?php  echo $set_width-10?>px; /*width of content DIVs within slider. Total width should equal slider's inner width (390+5+5=400) */
height: 100%;
...
<?php 
	...
	if (tmq_num_rows($showcase)!=0) {
		...
		?><div class="contentdiv"><?php 
			...
			echo "<img src='$img[url]' width=100% height=100% border=0>";
			echo "</A>";
