...
$tagsubj=tmq_dump("bkedit","fid","fid","where subj='on' ");
$tagsubj=implode(',\'\n\',',$tagsubj);
...
while ($r=tmq_fetch_array($s)) {
	...
	while (list($k,$v)=each($data)) {
		$word=$v;
		$tmp=tmq("select id from index_subjextract where word1='".addslashes($word)."' ",false);
		$tmp=tmq_num_rows($tmp);
		if ($tmp==0 && $word!="") {
			tmq("insert delayed into index_subjextract set word1='".addslashes($word)."',usoundex='".usoundex_get($word)."' ,importdt='$importdt',mid='$r[ID]',importid='$ID' ");
		}
	}
}
