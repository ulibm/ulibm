...
$sep_rec=barcodeval_get("importer_bkitems-sep_rec");
$sep_field=barcodeval_get("importer_bkitems-sep_field");
$cover_field=barcodeval_get("importer_bkitems-cover_field");
$file=barcodeval_get("importer_bkitems-file");
$fpath=$dcrs."/_input/import/$file";
//echo $fpath;
if (!file_exists($fpath)) {
	?><CENTER><?php  echo getlang("หาไฟล์ที่ท่านเลือกไม่พบ กรุณา <A HREF='index.php'>คลิกที่นี่</A> เพื่อกลับไปเลือกไฟล์อีกครั้ง::l::Your choosed file not found <A HREF='index.php'>please click here</A> to continue"); ?></CENTER><?php 
...
tmq("delete from importer_bkitems_tmp");
	for ($i=0;$i<=$reclistcount;$i++) {
		$fieldcount=count($reclist[$i]);
		if ($fieldcount>50) {
			$fieldcount=50;
		}
		$s="insert into importer_bkitems_tmp  set id='' ";
