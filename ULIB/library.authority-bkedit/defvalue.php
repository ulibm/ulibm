...
if ($saveedittag!="" && $savetp!="") {
	...
	tmq(" delete from bkedit_authority_defval where template='$savetp' and tag='$saveedittag'");
	if (trim($valueit)!="") {
		tmq("insert into bkedit_authority_defval set template='$savetp' , tag='$saveedittag', value='$valueit' ");
	}
}
if ($edittag!="" && $tp!="") {
...
$chk=tmq("select * from bkedit_authority_defval where template='$tp' and tag='$edittag' ",false);
$chk=tmq_fetch_array($chk);
...
Serif">
<a href="../root.marctemplate/index.php"  class=a_btn><?php  echo getlang("จัดการชุดฟอร์มการแก้ไขข้อมูล::l::Manage Marc template"); ?></a>  : 
...
$chk=tmq("select * from bkedit_authority_defval where template='$marctemplatekey' and tag='$row[fid]' ",false);
$chk=tmq_fetch_array($chk);
