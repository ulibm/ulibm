<?php 
	...
	$statdb[badd][name]="เพิ่มรายการใหม่::l::Add new member";
	$statdb[badd][sql]="add new member%";
	$statdb[bedit][name]="ปรับปรุงข้อมูล::l::Update member";
	$statdb[bedit][sql]="update member%";
	$statdb[bdel][name]="ลบข้อมูล::l::Delete member";
	$statdb[bdel][sql]="delete member%";

	$statdb[ftuploadcam][name]="เพิ่ม ภาพสมาชิก โดยกล้อง::l::Add photo using camera";
	$statdb[ftuploadcam][sql]="upload photo by camera%";
	$statdb[ftdel][name]="ลบไฟล์ภาพสมาชิก ::l::Delete Photo";
	$statdb[ftdel][sql]="Delete Photo%";
	$statdb[ftupload][name]="เพิ่มภาพสมาชิก::l::Add Photo";
	$statdb[ftupload][sql]="upload photo%";
...
for ($i=1;$i<=31;$i++) {
	...
	<?php  
	if (floor($tmp[$i])!=0) {
	  echo "<a href='mem.detail.php?y=$USEYEA&m=$USEMON&d=$i&libtoview=$libtoview&roomtoview=$roomtoview&edittype=".base64_encode($v[sql])."'>";
	}
