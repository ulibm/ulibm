...
?><BR>
                                <?php 
                                pagesection("$tmp");
                                    $sql1="SELECT distinct libsite  FROM member";
                                    $sql2="$sql1";
                                    //echo $sql2;
...
$memnum= number_format(tmq_num_rows(tmq("select * from member where libsite='$row[libsite]' ")));
echo $memnum;
echo "</td>";
...
<a href='man_del.php?ID=$ID&issave=yes' onclick='return confirm(\" $cfrm\") && confirm(\"".getlang("กรุณายืนยันอีกครั้ง หากต้องการลบสมาชิกทั้งหมด $memnum คน ในสาขาห้องสมุดนี้::l::Please confirm to delete $memnum member in this library campus")."\")'><B>".getlang("ลบ::l::Delete")."</B></a>";
										} else {
											echo "-";
										}
                                        echo "</td>";
										...
										if ($memnum!=0) {
											$rand1=randid();
											$rand2=randid();
                                            echo "<A HREF='../library.mmanbyroom/export.php?libsiteid=$row[libsite]&mode=all&$rand1' class='a_btn smaller2' target=_blank>ทั้งหมด</A> 
											<A HREF='../library.mmanbyroom/export.php?libsiteid=$row[libsite]&mode=report&$rand2' class='a_btn smaller2' target=_blank>สมาชิกที่มีปัญหา</A>";
										} else {
											echo "-";
										}
                                        echo "</td>";                                        echo "</tr>";
...
//echo $_pagesplit_btn_var;
   ...
   while ($r=tfa($s)) {
      $chk=tmq("select distinct concat(dat,'-',mon,'-',yea) as edata, count(UserAdminID) as cc from member where LIBSITE='$r[code]' group by concat(dat,'-',mon,'-',yea)");
      ...
      if ($chkcc==1 && $chkr[cc]>0) {
         ...
         if ($dat2<time()) {
            echo "<font color=darkred>".getlang("สมาชิกทุกคนใน $r[name] ($chkr[cc] คน) หมดอายุสมาชิกแล้ว::l::All member in $r[name] ($chkr[cc]) expired") . " ".ymd_datestr($dat2,'shortd');
                  echo "</font><BR>";
         } elseif ($dat2<(time()+(60*60*24*30))) {
            echo getlang("สมาชิกทุกคนใน $r[name] ($chkr[cc] คน) จะหมดอายุสมาชิกใน 1 เดือน::l::All member in $r[name] ($chkr[cc]) will expired in 1 month"). " ".ymd_datestr($dat2,'shortd');
              echo "<BR>";
         }
      }
