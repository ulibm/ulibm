...
if ($issave=="yes") {
	...
	tmq("insert into setreturndate set loginid='$useradminid' ,
	...
	yea='$yea'");
	$newid=tmq_insert_id();
	$s=tmq("select * from checkout where returned='no' ");
	$newdt=ymd_mkdt($dat,$mon,$yea-543);
	while ($r=tfa($s)) {
		$olddt=ymd_mkdt($r[edat],$r[emon],$r[eyea]-543);
		//echo "$olddt/$newdt<br>";
		if ($olddt>$newdt) {
			tmq("insert into setreturndate_sub set pid='$newid',
			member='$r[hold]',
			origid='$r[id]',
			bcode='$r[mediaId]',
...
<CENTER>
<B><?php  echo getlang("กำหนดวันคืนทรัพยากรทั้งหมดที่ถูกยืมออกจากห้องสมุดในขณะนี้ (เฉพาะรายการที่มีกำหนดการส่งหลังจากวันที่กำหนด)<br>กรุณาตรวจสอบให้แน่ใจก่อนทำการตั้งค่า เพราะการตั้งค่าไม่สามารถยกเลิกได้::l::Force set due date for checked out materials (only items pass this new due date)<br>Please re-check this operation, this operation cannot be undone."); ?>
...
function local_det($wh) {
	...
	$cc=tmq("select * from setreturndate_sub where pid='$wh[id]' ");
	$num=tnr($cc);
