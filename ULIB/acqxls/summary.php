...
?><?php 
pagesection("สรุปตามงบประมาณ (เฉพาะรายการที่สั่งซื้อ)");
$s=tmq("select distinct budget from acqn_sub where (stat='ordering' or stat='bookrecieve')  and pid='$pid' group by budget");
?><table class=table_border align=center width=780>
<tr>
	<td class=table_head>งบประมาณ</td>
...
while ($r=tfa($s)) {
	//printr($r);
	?><tr>
	<td class=table_td><?php  echo $bgdb[$r[budget]];?></td>
	<td class=table_td align=right><?php  
	$p=tmq("select sum(pricenet) as pp,count(id) as cc from acqn_sub where (stat='ordering' or stat='bookrecieve' )  and budget='".addslashes($r[budget])."' and pid='$pid' ");
	$p=tfa($p);
	echo $p[cc];
	$sum1=$sum1+$p[cc];
	$sum2=$sum2+$p[pp];
	...
	<td class=table_td align=right><?php  
	$p=tmq("select sum(pricenet) as pp,count(id) as cc from acqn_sub where (stat='bookrecieve' )  and budget='".addslashes($r[budget])."' and pid='$pid' ");
	$p=tfa($p);
	if ($p[cc]==$lastc) {
...
<?php 
pagesection("สรุปตามบริษัท (เฉพาะรายการที่สั่งซื้อ)");
...
<tr>
	<td class=table_td >
			...
			<tr>
				<td sclass=table_head2><b>บริษัท</b></td>
