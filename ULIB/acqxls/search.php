...
while (list($k,$v)=each($kwa)) {
	...
	$s=tmq($smaster);
	if (tnr($s)!=0) { echo "ค้นพบในระบบจัดหา ".tnr($s)." รายการ<BR>";} else {
		echo "<FONT  color=darkgreen style='font-size:15; font-weight:bold;'>ไม่พบข้อมูลซ้ำซ้อนในระบบจัดหา</FONT>";
	}
	...
	$smaster="select * from index_db where kw like '%$v%'  order by id asc ";
	$s=tmq($smaster);
	if (tnr($s)!=0) { echo "ค้นพบในฐานข้อมูล ".tnr($s)." รายการ<BR>";} else {
		echo "<FONT  color=darkgreen style='font-size:15; font-weight:bold;'>ไม่พบข้อมูลซ้ำซ้อนในฐานข้อมูล</FONT>";
	}
if (tnr($s)>10) { echo "แสดงผลการค้นหา 10 รายการแรก<BR>";}
	?><TABLE width=700 align=center border=0 cellpadding=0 cellspacing=0>
		<tr>
			<td class=table_head>
			<a href="sub.php?pid=<?php  echo "".$dcrURL."advsearching.php?bool%5B%5D=%5B%5BAND%5D%5D&kw%5B%5D=$v&searchopt%5B%5D=kw";?>" target=_blank>คลิกเพื่อค้นหา</a></td>
		</tr>	<TR>
		<TD><?php 
		$countdsp=0;
	while ($r=tfa($s) ) {
		...
		<tr>
			<td>&nbsp;&nbsp;&nbsp; &bull;<a href="<?php  echo $dcrURL?>dublin.php?ID=<?php  echo $r[mid]?>" target=_blank><?php 
