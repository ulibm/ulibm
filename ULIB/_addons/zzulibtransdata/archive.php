...
class tar_file extends archive
{
	...
	function extract_files()
	{
		...
		if ($fp = $this->open_archive())
		{
            ...
            $file['name'] = strval(str_replace("\0", "", $file['name']));
            ...
            @chmod($file['name'],0777);
				...
				{
               if ($file['stat'][7] !=0) { //source file size 0
   					if (!fwrite($new, fread($fp, $file['stat'][7]))) {
                     ?><h1>Problem writing file, reloading</h1>
                     for <?php echo $file[name];?>
                     <script> self.location.reload(); </script>
                     <?php
                     die;
   					}
               }
               fread($fp, (512 - $file['stat'][7] % 512) == 512 ? 0 : (512 - $file['stat'][7] % 512));
               /*
					if (!fread($fp, (512 - $file['stat'][7] % 512) == 512 ? 0 : (512 - $file['stat'][7] % 512))) {
                  //problem reading file = reload page
                  //printr($file); 
                  if ($file[name]=="config.inc.php") {
                     ?><h1>Problem reading archives for <?php echo $file[name];?>, but it's ok</h1><?
                  } else {
                     if (strtolower(substr($file[name],-4))!=".php") {
                     ?><h2>Problem reading misc. file <?php echo $file[name];?>, but it's ok</h2><?
                     } else {
                        ?><h1>Problem reading archives, reloading</h1>
                        for <?php echo $file[name];?>
                        <script> self.location.reload(); </script>
                        <?php
                        die;
               ...
               */
               ...
               $chkpath=$this->options['basedir']."/".$file[name];
               
               $chkpatha=stat($chkpath);
               //echo $chkpath;
               //printr($chkpatha);
               //printr($this);
               ?><h2>Could not open {<?php echo $file['name'];?>} for writing</h2>
                  <?php
                  die;
					continue;
				}
