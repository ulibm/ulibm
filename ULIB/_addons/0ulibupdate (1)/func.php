...
function local_archiveit_getinfo($dir,$jobfor="info") {
	global $dcrs;
	if (!file_exists($dir)) {
		return Array();
	}
	...
	//echo $dir."<BR>";die;;
	if ($jobfor=="download") {
		$tmpdir=$dcrs."_addons/0ulibupdate/_download/";
		$tmpfile=$dcrs."_addons/0ulibupdate/_download/$randid.tgz";
	} else {
		$tmpdir=$dcrs."_addons/0ulibupdate/_tmp/";
		$tmpfile=$dcrs."_addons/0ulibupdate/_tmp/$randid.tgz";
	}
   ...
   while (list($k,$v)=each($fss)) {
	...
	$b->add_files($filestozip);
	$b->exclude_files($tmpdir."*.log");
	$b->exclude_files($tmpdir."*.tmp");
	$b->exclude_files($tmpdir."*.bak");
	//printr($b->exclude);
	//$b->make_list();
	//printr($b->files);
	//echo count($b->files);
	...
	$res=Array();
	$res[hashdata]=md5_file($tmpfile);
	$res[size]=filesize($tmpfile);
	$res[dlfilename]=$randid;
	if ($jobfor!="download") {
		@unlink($tmpfile);
		@unlink($tmpfile.".tmp");
	}
