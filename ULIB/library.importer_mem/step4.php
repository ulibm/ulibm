<?php 
	; 
        ...
        mn_lib();
if ($resetmemory=="yes") {
	tmq("delete from barcode_val where classid like 'importmemory-member-%' ");
	tmq("delete from barcode_valmem where classid like 'importmemory-member-%' ");
}
...
function local_form_sub($wh) {
	...
	<?php 
	$tmp=barcodeval_get("importmemory-member-$wh");
	if ($tmp!="") {
...
<?php 
$s=tmq("select * from importer_mem_tmp limit 0,8");
...
while ($r=tfa($s)) {
	...
	<TD><?php  local_form2("cust[".$r[fid]."]");?></TD>
</TR><?php 
}
?>
<TR>
