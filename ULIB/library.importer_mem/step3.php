...
$sep_rec=barcodeval_get("importer_mem-sep_rec");
$sep_field=barcodeval_get("importer_mem-sep_field");
$cover_field=barcodeval_get("importer_mem-cover_field");
$file=barcodeval_get("importer_mem-file");
$fpath=$dcrs."/_input/import/$file";
//echo $fpath;
if (!file_exists($fpath)) {
	?><CENTER><?php  echo getlang("หาไฟล์ที่ท่านเลือกไม่พบ กรุณา <A HREF='index.php'>คลิกที่นี่</A> เพื่อกลับไปเลือกไฟล์อีกครั้ง::l::Your choosed file not found <A HREF='index.php'>please click here</A> to continue"); ?></CENTER><?php 
...
$reclist=fso_file_importmelt($fpath,50000,10000000,$sep_rec,$sep_field,$cover_field);

$reclistcount=count($reclist);
tmq("delete from importer_mem_tmp");
	for ($i=0;$i<=$reclistcount;$i++) {
		$fieldcount=count($reclist[$i]);
		if ($fieldcount>30) {
			$fieldcount=30;
		}
		$s="insert into importer_mem_tmp  set id='' ";
