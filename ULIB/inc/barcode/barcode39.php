<?php  //พ
    ...
    */
    ...
    //-----------------------------------------------------------------------------
    function Barcode39($barcode, $width, $height, $quality, $format, $text ,$mode="")
        {
        ...
        if (($NarrowBar == 0) || ($NarrowBar == $WideBar) || ($NarrowBar == $QuietBar) || ($WideBar == 0) || ($WideBar == $QuietBar) || ($QuietBar == 0))
            ...
            return OutputImage($im, $format, $quality,$mode);
            exit;
            }
    ...
    //-----------------------------------------------------------------------------
    function OutputImage($im, $format, $quality,$outputmode)
        {
			if ($outputmode!="") {
				switch ($format)
					{
					case "JPEG":
						ImageJPEG($im, "$outputmode", $quality);
						break;
					case "PNG":
						ImagePNG ($im ,$outputmode);
						break;
					case "GIF":
						ImageGIF ($im ,$outputmode);
						break;
					}
			} else {
				switch ($format)
