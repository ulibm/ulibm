...
@include("$dcrs/inc/rss2html/FeedForAll_rss2html_pro.php");
...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (isset($outputCacheTTL) && function_exists("FeedForAll_scripts_readOutputCacheFile") && (($cacheContents = FeedForAll_scripts_readOutputCacheFile($XMLfilename, $TEMPLATEfilename)) !== FALSE)) {
    ...
    if (strstr(trim($XML), "<?php xml") === FALSE) {
      $XML = "<?php xml version=\"1.0\"?>\n$XML";
    }
    $XML = strstr(trim($XML), "<?php xml");
...
?><br /><a href="<?php  echo $XMLfilename?>" class=feedbtn target=_blank><img align=absmiddle src="<?php  echo $dcrURL?>/neoimg/feed-icon-14x14.png" border=0> <?php  echo getlang("Feed");?></a>