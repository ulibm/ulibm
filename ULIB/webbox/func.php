<?php  //พ
//echo "--$deftablayout - $deftabmodule -- ";
...
nImg.src        = '<?php echo $dcrURL; ?>webbox/drag_drop_poof.gif';
...
function mouseUp(ev){
	...
	///if(Demos[0] || Demos[4]){
		...
		@reset($tabs);
		while (list($k,$tabi) = each($tabs)) {
		?>
				DragContainerObj=getobj("DragContainer<?php  echo $tabi?>");
				//alert(DragContainerObj.childNodes.length);
				str2save=str2save+"&tab[<?php  echo $tabi?>]="
		...
		?>
		getobj("POSSAVER").src="<?php  echo $dcrURL?>webbox/man.saveboxpos.php?curtab=<?php  echo $deftab?>"+str2save;
		//alert(str2save);
...
window.onload = function(){
	...
	for ($tabi=1;$tabi<=floor($tablayouts[$deftablayout][colnum]);$tabi++) {
	?>document.getElementById('DragContainer<?php  echo $tabi?>')<?php 
		if ($tabi<floor($tablayouts[$deftablayout][colnum])) {
			echo ",";
		}
	}
	$s=tmq("select * from webbox_box where tab='$deftab' and col='TOP' ",false);
	$s2=tmq("select * from webbox_box where tab='$deftab' and col='BOTTOM' ",false);
	if (tmq_num_rows($s)!=0 || tmq_num_rows($s2)!=0 || loginchk_lib('check')==true) {
		if (floor($tablayouts[$deftablayout][colnum])!=0) {
			echo ",";
		}
		?>document.getElementById('DragContainerTOP'),
			document.getElementById('DragContainerBOTTOM'),
			document.getElementById('DragContainerMENU')<?php 
	}
	?>
	);/*
		...
		<?php 
$s=tmq("select * from webbox_box where tab='$deftab' ",false);
while ($r=tmq_fetch_array($s)) {

	for ($tabi=1;$tabi<=floor($tablayouts[$deftablayout][colnum]);$tabi++) {
		?>
		//addDropTarget(document.getElementById('webbox<?php  echo $r[id]?>'),  'DragContainer<?php  echo $tabi?>');
		<?php 
	}
}	
	?>
...
//-->
</SCRIPT><iframe width=300 src="" height=300 ID='POSSAVER' frameborder=no style="display:none;"></iframe>