<?php 
   ...
   //echo substr($title,0,10);
	if (substr($title,0,10)=="permalink-") {
      //echo $title;
      $tmptitle=tmq("select * from webpage_wiki where id='".floor(substr($title,10))."' and hasdata='yes' ",false);
      $tmptitler=tfa($tmptitle);
      $title=$tmptitler[title];
      if (trim($title)=="") {
         $title="wiki:home";
      }
	}
	...
	if ($ismanager==true && $deleteit=="yes" && $title!="") {
		...
		html_xpbtn(getlang("ลบ::l::Delete")."&nbsp;,$dcrURL"."index.php?webboxload=yes&title=$title&realdeleteit=yes,red".
"::".getlang("ยกเลิก::l::Cancel")." ,$dcrURL"."index.php?webboxload=yes&title=$title,green");
		?></CENTER><BR><?php 
	}
...
?>
	...
	<TABLE width=200 cellpadding=0 cellspacing=0 border=0 align=right><TR><TD  align=right>
		<?php html_xpbtn(getlang("บทความทั้งหมด/ค้นหา::l::All articles/Search")." ,$dcrURL"."index.php?webboxloadwikisearch=yes,green");?>
	</TD></TR></TABLE>
...
if (tmq_num_rows($wk)==0) {
	...
	$authstr=getlang("โดย::l::By")." " . html_library_name($wk[loginid])." เมื่อ ".ymd_datestr($wk[dt]);
   $authstr.=" <a href='$dcrURL"."index.php?webboxload=yes&title=permalink-$wk[id]' target=_blank TITLE='Permalink'>P</a>";
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////editing
	...
	if ($editit=="yes") {
		...
		?><TABLE width=100%><TR><TD align=right><?php html_xpbtn(getlang("ยกเลิกการแก้ไข::l::Cancel Editing")." ,$dcrURL"."index.php?webboxload=yes&title=$title,green");?></TD></TR></TABLE><?php 
...
?><BR><FORM METHOD=POST ACTION="<?php  echo $dcrURL?>">
<TABLE width=100% cellpadding=0 cellspacing=0 border=0>
<INPUT TYPE="hidden" NAME="savetopic" value="yes">
<INPUT TYPE="hidden" NAME="webboxload" value="yes">
<INPUT TYPE="hidden" NAME="title" value="<?php  echo $title?>">
	<TR>
		<TD><?php 
	$html_htmlarea_genwiki="yes";
	$html_htmlarea_width=650;
...
<?php 
if ($editit=="yes") {
	?><label><INPUT TYPE="checkbox" NAME="minichange" value="yes" checked> <?php 
		echo getlang("เป็นการเปลี่ยนแปลงเล็กน้อย::l::Minor Change ");
	echo "</label>";
}	
?>	
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////view wiki
	...
	if ($ismanager==true) {
		...
		<tr bgcolor = dddddd>
			<td align=right><?php 
		if (editperm_chk($titlekey)==true) {
			html_xpbtn(getlang("ลบ::l::Delete")."&nbsp;,$dcrURL"."index.php?webboxload=yes&title=$title&deleteit=yes,red".
			"::".getlang("แก้ไข::l::Edit")." ,$dcrURL"."index.php?webboxload=yes&title=$title&editit=yes,green");
		} else {
			echo getlang("คุณไม่มีสิทธิ์ลบหรือแก้ไขรายการนี้::l::Your login have no permission to modify this article;");
		}
		?></td>
   ...
   ?><table width = "100%" align=center border = "0" cellspacing = "1" cellpadding = "3" bgcolor = "#bbbbbb" >
	...
	if ($logedinonlychk !== false && loginchk_lib("check")==false && $_memid=="") {
			...
			$backto="$dcrURL"."index.php?webboxload=yes&title=".urlencode(urlencode($title));
			echo "<BR><BR>";
			form_member_login();
			echo "<BR><BR><BR><BR>";
			echo "</DIV>";
			...
			if (getval("_SETTING","wikiusemodrewrite")=="yes") {
				$parser2->reference_wiki = "$dcrURL"."wiki/";
			} else {
			...
			/*$output=str_replace("<br /><br /><br />","<br />",$output);
			...
			*/
			...
			if (count($tagged)>0) {
				...
				while (list($taggedk,$taggedv)=@each($tagged)) {
					?><A HREF="<?php 
				if (getval("_SETTING","wikiusemodrewrite")=="yes") {
					$taggedvlink=str_replace(":","--namespace--",$taggedv);
					echo "$dcrURL"."wiki/".$taggedvlink;
				} else {
					echo "$dcrURL";?>index.php?webboxload=yes&title=<?php  echo urlencode($taggedv);
				}
					?>" class=smaller><?php 
	...
	while ($linktor=tmq_fetch_array($linkto)) {
		?><A HREF="<?php 
				if (getval("_SETTING","wikiusemodrewrite")=="yes") {
					$linktorlink=str_replace(":","--namespace--",$linktor[title]);
					echo "$dcrURL"."wiki/".$linktorlink;
				} else {
					echo "$dcrURL";?>index.php?webboxload=yes&title=<?php  echo urlencode($linktor[title]);
				}
				?>" class=smaller><?php 
