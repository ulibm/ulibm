...
$s=tmq("select SQL_CACHE id from acqn_sub where pid='$pid' ");
while ($r=tfa($s)) {
...
<tr>
	...
	<?php 
echo "กำหนดสถานะ: ";
		?>  <a href="bookrecieve.php?setstat=bookrecieve"
		target=_blank style="font-weight:bolder; color:<?php  echo $_s[bookrecieve][color];?>">หนังสือมาแล้ว</a><?php 
		?>  <a href="bookrecieve.php?setstat=reject"
		target=_blank style="font-weight:bolder;color:<?php  echo $_s[reject][color];?>">หนังสือขาดตลาด</a><?php 
			echo " (ตามหมายเลขอ้างอิง)";
?>	
...
<input type="hidden" name="pid" value="<?php  echo $pid; ?>">
	เลือกตามสำนักพิมพ์ 
	<?php 
	$ps=tmq("select SQL_CACHE distinct s_store, count(id) as cc from acqn_sub where pid='$pid'  group by s_store order by s_store");
...
function selectstorechange(wh) {
	...
	tmpobj.innerHTML="<iframe width=780 frameborder=0 height=39 src='chksubstore.php?pid=<?php  echo $pid;?>&store="+tmp+"' style=\"vertical-align : bottom;;\"></iframe><br>";
}		
	//-->
	</script>
</form>
...
if ($fftmode=="edit") {
	$_TMPACQSUGGESTORNAME=tmq("select * from acqn_sub where id='$ffteditid' ");
	$_TMPACQSUGGESTORNAME=tfa($_TMPACQSUGGESTORNAME);
}
if ($ffe_issave=="yes") {
	//print_r($_POST);
	$clog=tmq("select * from acqn_sub where $ffe_condition");
	$clog=tfa($clog);
	if ($clog["stat"]!=$ffdat["stat"] ) {
		//echo $clog["stat"]."!=".$ffdat["stat"];
		$now=time();
		tmq("insert into acqn_sub_clog set sub".trim($ffe_condition).",stat='".$ffdat["stat"]."',dt='$now' ");
	}
}
...
".stripslashes($wh[yea])."</a> 
	...
	if ($wh[titl]!="") {
		$s.= "<u><a href=\"http://lib3.msu.ac.th/search/?searchtype=t&SORT=D&searcharg=$wh[titl]\" target=_blank style='font-size:10px;' class=chkopaca>Title<img src='newwindow.png' width='8' height='8' border='0' ></a></u> ";
	}
	if ($wh[auth]!="") {
		$pos = strpos($wh[auth], ",");
		if ($pos === false) {
		...
		$s.= " <u><a href=\"http://lib3.msu.ac.th/search/?searchtype=a&SORT=D&searcharg=$wh[auth]\" target=_blank style='font-size:10px;' class=chkopaca>Author<img src='newwindow.png' width='8' height='8' border='0' ></a></u> ";		
	}
	if ($wh[isn]!="") {
		$s.= "<u><a href=\"http://lib3.msu.ac.th/search/i?SEARCH=$wh[isn]&sortdropdown=-&submit=Submit\" target=_blank style='font-size:10px; ' class=chkopaca>ISBN<img src='newwindow.png' width='8' height='8' border='0' ></a></u>";
	}
	if ($wh[titl]=="") {
