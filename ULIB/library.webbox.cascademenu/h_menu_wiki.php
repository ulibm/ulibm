...
if ($issave=="yes") {
	tmq("delete from webbox_topmenu_wiki where refid='$id' ");
	$url=addslashes($url);
	$now=time();
	tmq("insert into webbox_topmenu_wiki  set text='$text',refid='$id',dt=$now ");
}
$s=tmq("select * from webbox_topmenu_wiki where refid='$id' ");
$s=tmq_fetch_array($s);
