...
if ($issave=="yes") {
	tmq("delete from webbox_topmenu_content where refid='$id' ");
	$title=addslashes($title);
	viewdiffman_add("menucontent","$id",$body);
	$body=addslashes($body);
	$now=time();

	tmq("insert into webbox_topmenu_content  set title='$title',body='$body',refid='$id',dt=$now,globalslideshow='$globalslideshow'  ");
}
$s=tmq("select * from webbox_topmenu_content where refid='$id' ");
$s=tmq_fetch_array($s);
...
<tr valign = "top">
 ...
 <tr valign = "top">
  ...
  ?><BR>
    <?php 
	frm_globalupload("webbox_topmenucontents-$id","body");
  ?></td>
 </tr>
<tr valign = "top">
