...
$parent=tmq("select * from memcard where code='$pid' ");
$parent=tfa($parent);
//printr($parent);
...
<div ID="paperdiv" style="position:absolute; background-color: #ffffff; display: block; width: <?php  echo floor($parent[w]*$_MMTOPX)?>px; height: <?php  echo floor($parent[h]*$_MMTOPX)?>px;
...
<?php 
$s=tmq("select * from memcard_sub_i where pid='$origpid' order by ordr ");
$rida=Array();
while ($r=tfa($s)) {
	$pos=explode(";",$r[pos]);
	//printr($pos);
...
<iframe ID="IFSAVER" name="IFSAVER" xstyle="display:none"></iframe>