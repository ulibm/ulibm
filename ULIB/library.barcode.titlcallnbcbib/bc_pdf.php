...
$percolumn=barcodeval_get("BARCODE-titlcallnbcbib-perpage");///
$allbc=barcodeval_get("BARCODE-titlcallnbcbib-allbc");///
...
$NRow = count($result);
if($NRow >0) { 
	$iwidth=barcodeval_get("BARCODE-titlcallnbcbib-iwidth");
	$iheight=barcodeval_get("BARCODE-titlcallnbcbib-iheight");
	$width=barcodeval_get("BARCODE-titlcallnbcbib-width");
	$height=barcodeval_get("BARCODE-titlcallnbcbib-height");
	$str=barcodeval_get("BARCODE-titlcallnbcbib-str");
	...
	$colcount=1;
	 while(list($k,$v) = each($result)) {
		...
		if ($colcount==1) {
		 ...
		 //$tmp=tmq("select * from media where ID='$rowbc' ");
		 //$tmp=tmq_fetch_array($tmp);
		$callnum=marc_getcalln($rowbc);
		$title=marc_gettitle($rowbc);
		if (strlen($title)>27) {
			$title=substr($title,0,25)."..";
		}
		if (strlen($callnum)>27) {
			$callnum=substr($callnum,0,25)."..";
		}
...
//		$pdf->Image("$img",$startx+51+5,$starty+10+$height,$width-51,$height);
		...
		if ($text==1) {
			...
			$pdf->Text($startx+12,$starty+ceil($height/8)+65,trim("bib:$ID") );
		}
		$pdf->SetFont('Tahoma','',12);
		$pdf->Text($startx+2,$starty+ceil($height/8)+10,iconvth(trim("$title") ));
