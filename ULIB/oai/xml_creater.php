...
*/
function local_showDOMNode(DOMNode $domNode) {
    foreach ($domNode->childNodes as $node)
    {
        //print $node->nodeName.':'.$node->nodeValue."\n";
        $note->nodeValue="s";
        if($node->hasChildNodes()) {
            local_showDOMNode($node);
        }
    }    
}
...
class ANDS_XML {
   ...
   */
	...
	/*$config = $pr->getElementsByTagName( "dc:title" );
foreach( $config as $global )
{
   printr($global);
}
		die("here");*/
      //local_showDOMNode($pr);
      /*
$descriptions = $pr->getElementsByTagName('dc:title');
