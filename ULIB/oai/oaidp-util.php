	...
	if(isset($checkList['required'])) {
		for($i = 0; $i < count($checkList["required"]); $i++) {
			...
			// echo "caught\n";
				...
				if(false && isset($args['metadataPrefix'])) { ///
...
{
   //echo "formatDatestamp($datestamp)";
   if (is_numeric($datestamp)) {
   	return date("Y-m-d\TH:i:s\Z",($datestamp));
   } else {
...
function idQuery ($metadPrefix = "rif", $id = '')
{
	...
	if ($SQL['set'] != '') {
		...
		$query = 'select '.$SQL['identifier'].','.$SQL['datestamp'].','.$SQL['set'].' FROM '.$SQL['table']. " WHERE 1";
	} else {
		$query = 'select '.$SQL['identifier'].','.$SQL['datestamp'].' FROM '.$SQL['table']. " WHERE ".$SQL['metadataPrefix']." LIKE '%$metadPrefix%'";///
		$query = 'select '.$SQL['identifier'].','.$SQL['datestamp'].' FROM '.$SQL['table']. " WHERE 1";
	}
