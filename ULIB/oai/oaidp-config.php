...
// MUST (only one)
// please adjust
$identifyResponse["repositoryName"] = getval("oaiconfig","repositoryName");
if ($identifyResponse["repositoryName"]=="") {
   $identifyResponse["repositoryName"]="repositoryName";
}
// For ANDS to harvest of RIF-CS, originatingSource is plantaccelerator.org.au
// $dataSource = "plantaccelerator.org.au";
define('DATASOURCE',getval("oaiconfig","DATASOURCE"));

// do not change
define('MY_URI','http://'.$_SERVER['SERVER_NAME'].":".$_SERVER['SERVER_PORT'].$_SERVER['SCRIPT_NAME']);
...
//$identifyResponse["earliestDatestamp"] = '201-01-01';
$identifyResponse["earliestDatestamp"] = '2010-01-01';
...
$adminEmail			= explode(",",barcodeval_get("mailsetting-ReturnPath")); 
//printr($adminEmail);
if (trim(barcodeval_get("mailsetting-ReturnPath"))=="") {
$adminEmail			= explode(",","root@localhost,admin@Localhost"); 
}
@reset($adminEmail);
...
// please adjust
$repositoryIdentifier = getval("oaiconfig","repositoryIdentifier"); 
...
/*$SETS = 	array (
          ...
          <dc:description>This set contains metadata describing electronic music recordings made during the 1950ies</dc:description>
			...
			/*
$s=tmq("select * from collections where classid<>'' order by name");
$sres=Array();
while ($r=tmq_fetch_array($s)) {
	$sresi=count($sres);
	$sres[$sresi]=Array();
	$sres[$sresi]["setSpec"]="class:$r[classid]";
	$sres[$sresi]["setName"]=iconvutf(getlang("$r[name]","en"));
	$sres[$sresi]["setDescription"]='<oai_dc:dc 
          xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/" 
          xmlns:dc="http://purl.org/dc/elements/1.1/" 
          xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
          xsi:schemaLocation="http://www.openarchives.org/OAI/2.0/oai_dc/ 
          http://www.openarchives.org/OAI/2.0/oai_dc.xsd">
          <dc:description>'.iconvutf(getlang("$r[descr]","en")).'</dc:description>
       </oai_dc:dc>';
}
	$sresi=count($sres);
	$sres[$sresi]=Array();
	$sres[$sresi]["setSpec"]="all";
	$sres[$sresi]["setName"]="All";
*/
$SETS=$sres;
//printr($SETS);
//printr($SETS);
...
//$DSN = "mysql:host=$DB_HOST;port=;dbname=$DB_NAME;user=$DB_USER;password=$DB_PASSWD";
//$DSN = "mysql://$DB_USER:$DB_PASSWD@$DB_HOST/$DB_NAME";
$DSN = "mysql:host=$DB_HOST;dbname=$DB_NAME; port=".getval("oaiconfig","connectiondbport");
//print_r($_SERVER);
//echo $DSN;
...
$charset = "iso8859-1";
$charset = getval("oaiconfig","charset");
