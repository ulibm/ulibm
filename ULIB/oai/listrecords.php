...
if (empty($errors)) {
	...
	//echo "--".tnr($r)."-";
 	if ($r===false) {
		...
		$num_rows = tnr($res);//rowCount($metadataPrefix, $extquery, $db);  
...
while ($countrec++ < $maxrec) {
	...
	//printr($record);
	//$setspec = str_remspecialsign($record["collist"]);
	//$setspec = "class:fiction";
  ...
  //debug_var_dump('status_deleted', $status_deleted);
	if($args['verb']=='ListRecords') {
		...
		//$outputObj->create_spatial_node("spatialspatialspatial", $datestamp,$setspec,$cur_record);
	// return the metadata record itself
		if (!$status_deleted) {
			...
			//var_dump($outputObj);
			//var_dump($outputObj);
			//$tmp=$outputObj->addChild($outputObj, "aa");
   		//$outputObj->working_node->setAttribute('type', "sd");
			//create_metadata($outputObj, $cur_record, "identifier", "setspec", $db);
			//create_metadata($outputObj, $cur_record, "identifier", "setspec", $db);
			//echo "HERE";
		}
