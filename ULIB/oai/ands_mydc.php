...
class ANDS_MYDC extends ANDS_RIFCS {
  ...
  function __construct($ands_response_doc, $metadata_node, $db) {
	...
	function create_obj_node($set_name, $key) {
		...
		if (@in_array($set_name,prepare_set_names())|| true) {
			try {
				// Get ori_id and which the original table is:
				$query = "select * from media where ID = '".$key."'";
			...
			$this->$processor($record['ori_table_name'],$record['ID']);	
			$this->create_identifier_node($key);
			$this->create_identifier_node('table=media+id='.$key);
		} else {
	 ...
	 */
 	private function create_all($table_name, $mid) {
		$db = $this->db;
		global $dcrURL;
		$query = ("select * from media where ID='$mid' ");
		//echo $query;
		$s=tmq($query,false);
		$r=tmq_fetch_array($s);
...
$r=marc_melt($mid); 
//printr($r);
		//$marcmelt=marc_melt($mid); 
		//printr($marcmelt);
		//$this->working_node->setAttribute('type', "whattype?");		
		//$this->create_rifcs_node('dc:title');
		
		//$this->create_namePart($c,'The Plant Accelerator Project '.$record['inter_no']);
		//$c = $this->create_name_node("dc:test");
		//$this->create_namePart($c,'The Plant Accelerator Project '.$record['inter_no']);
		//$this->create_rifcs_node('dc:collection','dataset');
   	//$c = $this->create_name_node();
   	//$this->create_namePart($c,'Data set of Plant Accelerator Project '.$prj_info['inter_no']);
   	//echo $mid."-x$mid x-".marc_gettitle($mid);
   	//title
		$tmpnew = $this->addChild($this->working_node, "dc:title");
		$ct = $this->working_node->ownerDocument->createCDATASection(iconvutf($r["tag245"]));
		//$ct = $this->working_node->ownerDocument->createCDATASection(iconvutf(marc_gettitle($mid)));
      $tmpnew->appendChild($ct);
      //creator
      $subj="\n".$r["tag100"]."\n".$r["tag700"]."\n".$r["tag110"]."\n".$r["tag710"];
	  $subj=iconvutf($subj);

      $subj=explodenewline($subj);
      $subj=arr_filter_remnull($subj);
      //printr($subj);
      @reset($subj);
      while (list($k,$v)=each($subj)) {
         if (trim($v)=="") continue;
         //echo "[$v]\n";
   		$tmpnew = $this->addChild($this->working_node, "dc:creator");
   		$ct = $this->working_node->ownerDocument->createCDATASection((dspmarc(substr($v,2))));
        $tmpnew->appendChild($ct);
      }
		///$tmpnew = $this->addChild($this->working_node, "dc:creator");
		///$ct = $this->working_node->ownerDocument->createCDATASection(iconvutf(marc_getauth($mid)));
      ///$tmpnew->appendChild($ct);
      //subj
      $subj=$r["tag650"];
      $subj=explodenewline($subj);
      ...
      while (list($k,$v)=each($subj)) {
         if (trim($v)=="") continue;
         //echo "[$v]\n";
   		$tmpnew = $this->addChild($this->working_node, "dc:subject");
   		$ct = $this->working_node->ownerDocument->createCDATASection(iconvutf(dspmarc(substr($v,2))));
         $tmpnew->appendChild($ct);
      }
      //date
      $getdt=substr($r["tag008"],7,4);
      //echo "[getdt=$getdt];\n";
      if (floor($getdt)>1000) {
         $this->addChild($this->working_node, "dc:date",iconvutf("$getdt"));
      }
      //lang
      $getlang=substr($r["tag008"],35,3);
      //echo "[getlang=$getlang];\n";
      if (strlen(trim("$getlang"))==3) {
         $this->addChild($this->working_node, "dc:language",iconvutf("$getlang"));
      }
	  //linkback
       $this->addChild($this->working_node, "dc:identifier",$dcrURL."dublin.php?ID=$mid");
	   //856
		$subj=$r["tag856"]."\n".$r["ulibtag856"];
		$subj=explodenewline($subj);
		$subj=arr_filter_remnull($subj);
		...
		while (list($k,$v)=each($subj)) {
			...
			$v=trim($v);
			 if (trim($v)=="") continue;
			 //echo "[$v]\n";
		   $this->addChild($this->working_node, "dc:identifier",$v);
		}
	   //type=text
       $this->addChild($this->working_node, "dc:type","text");

	   /*
	   if (trim($marcmelt[tag260_b])!="") {
		   $this->addChild($this->working_node, "dc:publisher",$marcmelt[tag260_b]);
	   }*/
      //$this->create_rifcs_node('activity','project');
      //$this->create_rifcs_node('dc:title',"test");
    //  $this->addChild($this->working_node, "dc:author");
		//$this->addChild($this->working_node, 'dc:title', "<![CDATA[".marc_gettitle($mid)." ]]>");
