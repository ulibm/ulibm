...
if ($issave=="yes") {
	tmq("delete from webmobile_menu_articledescr where refid='$id' ");
	$title=addslashes($title);
	$body=addslashes($body);
	$now=time();
	tmq("insert into webmobile_menu_articledescr  set text='$text',refid='$id' ");
	tmq("update webmobile_menu set orderby='$orderby' where id='$id' ");
}
$s=tmq("select * from webmobile_menu_articledescr where refid='$id' ");
$s=tmq_fetch_array($s);
...
<input type=hidden name="issave" value="yes">
 <tr valign = "top">
  ...
  <td width=600><?php  $scatename=tmq("select * from webmobile_menu where id='$id' ");
...
echo $scatename[name];
 ...
 <tr valign = "top">
  ...
  <td><?php  form_quickedit("text",$s[text],"html"); ?><BR>
    <?php 
	frm_globalupload("webmobile_menudescr-$id","text");
  ?></td>
 </tr>
