...
if ($issave=="yes") {
	tmq("delete from webmobile_menu_url where refid='$id' ");
	$url=addslashes($url);
	$now=time();
	tmq("insert into webmobile_menu_url  set url='$url',refid='$id',dt=$now,target='$target' ");
}
$s=tmq("select * from webmobile_menu_url where refid='$id' ");
$s=tmq_fetch_array($s);
if ($s[target]=="") {
	$s[target]="_top";
}
