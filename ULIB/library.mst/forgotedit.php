...
<form method = "post" action = "forgoteditsave.php" name = "webForm" enctype="multipart/form-data">                                <tr>
                                    <td class=table_head>
...
<td class=table_td>
	...
	<?php  echo getlang("ประเภท::l::Type"); 
		$resulttmp=tmq("select * from member_type where type='temp' ");
		$resulttmp=tfa($resulttmp);
	?> 
	<input type="hidden" name="type" value="temp">
	<b><?php  echo getlang($resulttmp[descr]);?></b>
</td>
</tr>
 <tr bgcolor = "#f3f3f3">
	...
	<td width = "73%">
	<font face = "MS Sans Serif" size = "2"><input type = text name = "credit" value="<?php  echo $row[credit]?>"> <INPUT TYPE="checkbox" NAME="isaddcreditfee" style="border:0" value="yes" checked> <?php 
	...
	<!--
		function member_expiresetfunc(wh) {
			...
			tmp=getobj('expmon');
			for (i=0;i<tmp.options.length;i++) {
			   //console.log(tmp.options[i].value+" "+curr_month);
				if (tmp.options[i].value==Math.floor(curr_month)) {
					tmp.options[i].selected=true
				}
			}
			tmp=getobj('expyea');
...
while (list($qnk,$qnv)=each($qn)) {
	?><a href="javascript:void(null);" onclick="tmp=getobj('xxx2'); tmp.value='<?php  echo stripslashes($qnv);?>'; " class="smaller2 a_btn"><?php  echo stripslashes($qnv);?></a> <?php 
	  ...
	  ?>		
		...
		$resulttmp=tfa($resulttmp);
	?> 
	<input type="hidden" name="room" value="<?php  echo ($resulttmp[id]);?>">
	<b><?php  echo getlang($resulttmp[name]);?></b>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td  class=table_head>
                                        ...
                                        <td class=table_td>
                                      <?php  
		$resulttmp=tmq("select * from major where istemp='YES' ");
		$resulttmp=tfa($resulttmp);
	?> 
	<input type="hidden" name="major" value="<?php  echo ($resulttmp[id]);?>">
			<b><?php  echo getlang($resulttmp[name]);?></b>	
                                        </td>
                                    </tr>
...
<?php 
                                    ...
                                    <tr>
                                        ...
                                        <td  class=table_td><input type="file" name="updatephoto" /> <A HREF="javascript:void(null)" onclick="window.open('./ulibcamcap/index.php?id=<?php  echo $ID?>','ulibcamcapwin','width=750,height=400')"><?php  echo getlang("ถ่ายภาพจากกล้อง::l::Capture from webcam")?></A><br />
																				<input type="checkbox" style="border:0" name="deloldphoto" value="yes"/> <?php  echo getlang("ลบภาพเก่า::l::Delete old photo");?>
																				<br />
																				<font class=smaller>
