...
*/
	return "$statusimg <A HREF='webpage.wiki.php?title=$wh[title]'> $dsptitle </A> $coverhtml";
}
...
class WikiParser {
	...
	function handle_sections($matches) {
		...
		<TR valign=top>
			<TD><font  style='display: inline; font-size:".($fontsizedb[$level]).";font-weight:bolder; '>{$content}</font></TD><TD align=right><A name='section".$this->section_count."' HREF='#topofwikipage' class=smaller2>".getlang("บนสุด::l::To to top")."</A></TD>
		</TR>
		</TABLE>\n\n";
	}
	...
	function handle_internallink($matches) {
		...
		if (tmq_num_rows($tmpchk)==0) {
			$tmp=sprintf(
				'<a href="%s"%s  style=\'color:darkred\'>%s</a>',
				$href,
