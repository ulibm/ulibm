...
function local_request($wh) {
	...
	<input type=hidden name='cardid' value='$wh[UserAdminID]'>
		<table>
	";
	$s=tmq("select * from oss_req where cardid='$wh[UserAdminID]' and stat='waitpayment' ");
	$sum=0;
	while ($r=tfa($s)) {
		...
		<td><a href='checkmatresult.php?id=$r[id]' notarget=_blank>".substr(dspmarc($tmp),0,60)."</a></td>
	...
	$res.="
	 ...
	 &bull;  รวม <b>".number_format($sum,2)."</b> บาท  <input type=submit value='คลิกที่นี่เพื่อทำการชำระ'></form>";
	return "$res";
}
/*
$dsp[5][text]="สถานะ";
...
$limit=" UserAdminID<>'' and UserAdminID in (select cardid from oss_req where stat='waitpayment' ) ";
$kw=trim($kw);
if ($kw!="") {
	$limit.="  and idcard = '$kw'";
}
