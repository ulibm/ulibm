...
$tmp=mn_lib();
$parent=tmq("select * from printtemplate_sub where code='".str_replace("FOOT::","",$pid)."' ");
$parent=tfa($parent);
...
if ($save_string!=0) {
	tmq("update printtemplate_sub_i set data='".addslashes($entertext)."',string_fontsize='$string_fontsize' where id='$save_string'");
}
...
if ($edit_string!=0) {
	...
	<tr>
		...
		$oldval=tmq("select * from printtemplate_sub_i where id='$edit_string' ");	
	$oldval=tfa($oldval);
...
if ($save_var!=0) {
	tmq("update printtemplate_sub_i set data='".addslashes($edit_var_data)."',string_fontsize='$string_fontsize' where id='$save_var'");
}
...
if ($edit_var!=0) {
	...
	<tr>
		...
		$oldval=tmq("select * from printtemplate_sub_i where id='$edit_var' ");	
	$oldval=tfa($oldval);
	$data= stripslashes($oldval[data]);
	form_quickedit("edit_var_data",$data,"foreign:-localdb-,printtemplate_var,code,name");
	?><br>
	Font Size  <input type="text" name="string_fontsize" value="<?php  
	$oldval[string_fontsize]=floor($oldval[string_fontsize]);
	if ($oldval[string_fontsize]==0) {
...
if ($save_image!=0) {
	$dr="$dcrs/_tmp/printtemplate_img/";
	//echo $dr;
...
if ($edit_image!=0) {
	...
	<tr>
		...
		<td class=table_td><?php 
	$oldval=tmq("select * from printtemplate_sub_i where id='$edit_image' ");	
	$oldval=tfa($oldval);
	if (file_exists($dcrs."_tmp/printtemplate_img/$oldval[id].jpg")) {
		?><img src="../_tmp/printtemplate_img/<?php  echo $oldval[id];?>.jpg?<?php  echo rand();?>" width=48 height=48 style="border-width:1;border-style:inset;"><?php 
	}
...
$c[4][field]="pos";
$c[4][fieldtype]="printtemplate_pos";
$c[4][descr]="";
$c[4][defval]="";

$c[5][text]="ประเภท::l::Type";
$c[5][field]="type1";
$c[5][fieldtype]="foreign:-localdb-,printtemplate_itype,code,name";
$c[5][descr]="";
$c[5][defval]="string";

$c[8][text]="ค่าตัวแปร::l::Variable";
$c[8][field]="varid";
$c[8][fieldtype]="foreign:-localdb-,printtemplate_var,code,name";
...
$c[10][fieldtype]="foreign:-localdb-,printtemplate_align,code,name";
$c[10][descr]=" ";
$c[10][defval]="J";
...
$itypedb=tmq_dump("printtemplate_itype","code","name");
...
function local_man($wh) {
	...
	if ($wh[type1]=="image") {
		...
		if (file_exists($dcrs."_tmp/printtemplate_img/$wh[id].jpg")) {
			$s.= "<img src=\"../_tmp/printtemplate_img/$wh[id].jpg?<?php  echo rand();?>\" width=48 height=48 style=\"border-width:1;border-style:inset;\">";
		}
...
?><center><a href="_preview.php?pid=<?php  echo $pid;?>" target=_blank class=a_btn>Preview</a></center><?php 
