...
function localmanage($wh) {
	...
	if ($wh[type]=="html") {
		return "<CENTER><A HREF='list_content.php?locate=homepage&id=$wh[id]'><B>".getlang("จัดการเนื้อหา::l::Manage")."</B></A></CENTER>";
	}
	if ($wh[type]=="rss") {
		return "<CENTER><A HREF='list_url.php?locate=homepage&id=$wh[id]'><B>".getlang("แก้ไขการเชื่อมโยง RSS::l::Edit RSS Url")."</B></A></CENTER>";
	}
	if ($wh[type]=="tab") {
		$c=tmq("select * from webpage_hpsidebar_tabs where locate='$wh[id]' ");
		$c=tmq_num_rows($c);
		return "<CENTER><A HREF='list_tab.php?locate=homepage&pid=$wh[id]'><B>".getlang("ข้อมูลที่จะแสดง::l::Data to display")."</B></A> ($c)</CENTER>";
	}
}

fixform_tablelister($tbname," 1 and locate='homepage' ",$dsp,"yes","yes","yes","locate=homepage",$c," ordr ");
